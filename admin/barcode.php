<?php //session_start();
	...
	$barcodeid=$_SESSION['barcodeid'];
        ...
        if($student_registration_no!=null && $barcodeid!=null && $accession_no!=null)
        {
            ...
            if($check_for_student==$student_registration_no)
            {
               //for checking remaining quantity
                ...
                if($remaining_quantity["remaining_quantity"]>=1)
                {
                    ...
                    .       "values ('$barcodeid','$student_registration_no','$accession_no',NOW())");
...
function barcodeTextboxFill(searchKey){
 ...
 require_once('db.php');
    ...
    if(isset($_POST["return_submit"]))
    {
        ...
        if($barcodeid !=null && $student_registration_no !=null && $accession_no !=null)
        {
           $row=mysql_query("select accession_no,registration_no,barcode_id from borrowing_info where accession_no='$accession_no'");
           $check=mysql_fetch_assoc($row);
           if($check['accession_no']==$accession_no 
             && $check['registration_no']==$student_registration_no
             && $check['barcode_id']==$barcodeid)
           {
                ...
                <?php
                 //email functionality
			          ...
			          //=========
			          //fetch email from student
			          $query=mysql_query("select email from student where registration_no='$student_registration_no'");
			          $row=mysql_fetch_assoc($query);
			          $email=$row['email'];
...
<body>
        ...
        <div class="panel-heading bg-primary" style="text-align:center"><h3><i class="glyphicon glyphicon-barcode"></i> Book Barcode Student Related</h3></div>
          	<div class="col-md-12">
