...
<?php //session_start();
    ...
    {   
        ...
        if($cnic!=null && $brcode_id!=null && $accession_no!=null)
        {
           //for checking that a student is registered or a valid student registration no 
            $check=mysql_query('select cnic,email from faculty where cnic = "'.$cnic.'"');
            ...
            if($check_for_faculty==$cnic)
            {
               //for checking remaining quantity
                ...
                if($remaining_quantity["remaining_quantity"]>=1)
                {
                    ...
                    .       "values ('$brcode_id','$cnic','$accession_no',NOW())");
                             mysql_query('update book_copy set status="Issued Faculty" where accession_no="'.$accession_no.'"');
            ...
            else
            {
              ?>
                    <div class="alert alert-warning alert-dismissable">
                    <a href="#" class="close" data-dismiss="alert" aria-label="close">×</a>
                    <strong>Warning!</strong> The faculty CNIC no is not valid.
                     </div>
               <?php  
            }
        }
     ...
     $.ajax({
        ...
        <div class="container content">
             <div class="row panel panel-primary add_student_well">
                  <div class="panel-heading bg-primary"><h3><i class="glyphicon glyphicon-share-alt"></i> Issue Book to faculty</h3></div>
          	
                    <div class="panel-body">
                       <div class="col-lg-12">

                           <form action="issue_book_faculty.php" method="post">
